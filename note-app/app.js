...
function playAlarmBeep() {
  try {
    ...
    // Ana gain (ses seviyesi kontrolü)
    const masterGain = ctx.createGain();
    masterGain.gain.setValueAtTime(0.8, ctx.currentTime);
    masterGain.connect(ctx.destination);

    // Alarm pattern: üç hızlı bip
    const pattern = [
      { freq: 1046, start: 0.0,  dur: 0.15 },
      { freq: 1046, start: 0.2,  dur: 0.15 },
      { freq: 1318, start: 0.4,  dur: 0.4  },
    ];
    ...
    pattern.forEach(({ freq, start, dur }) => {
      ...
      gain.gain.linearRampToValueAtTime(0.7, t + start + 0.01);
      gain.gain.setValueAtTime(0.7, t + start + dur - 0.02);
...
function startAlarm() {
  stopAlarm();
  playAlarmBeep();
  // Her 2.5 saniyede tekrar çal
  _alarmInterval = setInterval(playAlarmBeep, 2500);
  // Vibrasyon (Android/bazı cihazlar)
...
// iOS ses kilidini her etkileşimde aç
